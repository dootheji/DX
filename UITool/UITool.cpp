...
BOOL CUIToolApp::OnIdle(LONG lCount)
{
	...
	//	m_Tool.MessageProc(msg);
	//}
	m_Tool.TCoreFrame();
	syNode* selNode = m_Tool.CheckPickTest();
	if (selNode&&g_ActionInput.bRightClick==KEY_HOLD)
	{

		if (m_Tool.IsDrawAlpha)
			m_Tool.DrawAlpha(selNode);
		if(m_Tool.IsSetMapHeight)
			m_Tool.SetMapHeight(selNode);
		if (m_Tool.IsSetMapTex)
			selNode->SetMapTex(m_Tool.m_changedtexID);
		//if (m_Tool.IsSetbgTex)
			//selNode->SetbackgroundTex(m_Tool.m_changedtexID);
	}
